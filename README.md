...
План разработки
- [ ] Настроить базовый проект: структура проекта, файл зависимостей, безопасное хранение API-ключа и базовый эндпоинт.
- [ ] Написать скрипт, который читает данные о персонажах и сценариях из JSON-файлов, векторизует их и загружает в ChromaDB.
- [ ] Написать функцию, которая извлекает правила из DB_Rules (первый контур).
- [ ] Написать функцию, которая принимает историю чата, создает саммари (пока просто берет последние N сообщений), векторизует и ищет релевантные моменты в DB_History (второй контур).
- [ ] Написать функцию, которая извлекает активный чек-поинт (третий контур).
- [ ] Создать главную функцию, которая вызывает три контура, собирает из их результатов финальный промпт и отправляет его в Gemini API.
- [ ] Подключить Оркестратор к эндпоинту чата (простая фронтенд-часть).
...
